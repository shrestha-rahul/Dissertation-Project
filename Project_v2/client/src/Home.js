...
function Home() {
  ...
  useEffect(() => {
    axios.get(`${SERVER_URL}/get_transactions`).then((res) => {
      setTransactions(res.data);
    });

    axios.get(`${SERVER_URL}/get_budgets`).then((res) => {
      setBudgets(res.data);
    });

    axios.get(`${SERVER_URL}/get_reminders`).then((res) => {
  ...
  function remove_recent_budget(){
    axios.post(`${SERVER_URL}/remove_recent_budget`).then((res) => {
      axios.get(`${SERVER_URL}/get_budgets`).then((res) => {
        setBudgets(res.data);
      });
    });  
  }
  function remove_recent_transaction(){
    axios.post(`${SERVER_URL}/remove_recent_transaction`).then((res) => {
      axios.get(`${SERVER_URL}/get_transactions`).then((res) => {
        setTransactions(res.data);
      });
    });  
  }
  function remove_recent_reminder(){
    axios.post(`${SERVER_URL}/remove_recent_reminder`).then((res) => {
      axios.get(`${SERVER_URL}/get_reminders`).then((res) => {
        setUpcomingBills(res.data);
      });
    });  
              ...
              <div className="w-[40%] space-y-4 flex flex-col">
                ...
                <Card row="true" className="w-full space-x-2 grow">
                  ...
                  <div className="w-[50%] border-r-2 border-gray-800 pr-1">
                    <CardTitle path="/add_budget">Budgets <button type="button" onClick={remove_recent_budget} style={{marginLeft: "45px", color: "red"}}>x</button></CardTitle>
                    <Date />
                    <DounatChart />
                  </div>
              ...
              <div className="w-[30%] space-y-4 h-full flex flex-col ">
                ...
                <Card>
                  <CardTitle className="justify-center" path="/add_transaction">
                    Daily Transaction <button type="button" onClick={remove_recent_transaction} style={{marginLeft: "20px", color: "red"}}>x</button>
                  </CardTitle>
                  <CurrentDate />
                ...
                <Card className="grow ">
                  <CardTitle className="justify-center" path="/add_reminder">
                    Upcoming Bills <button type="button" onClick={remove_recent_reminder} style={{marginLeft: "45px", color: "red"}}>x</button>
                  </CardTitle>
